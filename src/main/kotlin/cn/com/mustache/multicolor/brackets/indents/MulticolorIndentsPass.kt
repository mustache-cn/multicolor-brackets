 ...
 * */
class MulticolorIndentsPass internal constructor(
    project: Project,
    editor: Editor,
    ...
    override fun doCollectInformation(progress: ProgressIndicator) {
        ...
        for (descriptor in myDescriptors) {
            ...
            rangesWithMulticolorInfo[textRange] =
                MulticolorIndentGuideRenderer.getMulticolorInfo(this.myEditor, textRange)
        }
    ...
    private fun nowStamp(): Long =
        if (isMulticolorIndentGuidesShown()) document.modificationStamp xor (EditorUtil.getTabSize(myEditor)
            .toLong() shl 24) else -1
    ...
    override fun doApplyInformationToEditor() {
        ...
        if (oldHighlighters != null) {
            ...
            while (curRange < myRanges.size && curHighlight < oldHighlighters.size) {
                ...
                when {
                    cmp < 0 -> {
                        newHighlighters.add(createHighlighter(mm, range, this.rangesWithMulticolorInfo))
                        curRange++
                    }
        ...
        DocumentUtil.executeInBulk(document, myRanges.size > 10000) {
            for (i in startRangeIndex until myRanges.size) {
                newHighlighters.add(createHighlighter(mm, myRanges[i], this.rangesWithMulticolorInfo))
            }
        }
    ...
    private fun buildDescriptors(): List<IndentGuideDescriptor> {
        if (!isMulticolorIndentGuidesShown()) return emptyList()
    ...
    */
                ...
                if (lineIndents[line] >= 0) {
                    ...
                    if (bottomIndent < topIndent) {
                        ...
                        if (BraceMatchingUtil.isRBraceToken(iterator, myChars, fileType) ||
                            tokenType != null &&
                            CodeBlockSupportHandler.findMarkersRanges(myFile, tokenType.language, nonWhitespaceOffset)
                                .isNotEmpty()
                        ) {
                            indent = topIndent
                        }
    ...
    companion object {
        private val INDENT_HIGHLIGHTERS_IN_EDITOR_KEY =
            Key.create<MutableList<RangeHighlighter>>("_INDENT_HIGHLIGHTERS_IN_EDITOR_KEY_")
        private val LAST_TIME_INDENTS_BUILT = Key.create<Long>("_LAST_TIME_INDENTS_BUILT_")

        private fun isMulticolorIndentGuidesShown(): Boolean {
            return MulticolorSettings.instance.isMulticolorEnabled && MulticolorSettings.instance.isShowMulticolorIndentGuides
        }
        ...
        private fun createHighlighter(
            ...
            return mm.addRangeHighlighter(
                ...
                customRenderer = MulticolorIndentGuideRenderer(rangesWithMulticolorInfo)
            }
        }
